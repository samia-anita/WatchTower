...
/**
 * File: backend/detectors/sqlInjection.js
 *
 * Purpose:
 *   Detects SQL injection attempts in parsed log events.
 *
 * Responsibilities:
 *   - Test each event's raw line, URL path, and user agent against SQL patterns
 *   - Score each detected threat with a risk weight based on pattern severity
 *   - Return structured threat objects for all matching events
 */
...
const SQL_PATTERNS = [
  ...
  { pattern: /%27|%22|%3D|%3B/i,                   weight: 65, name: 'URL-encoded SQL characters' },
];
...
function detectSQLInjection(events) {
  ...
  for (const event of events) {
    const textToCheck = [event.raw_line, event.path, event.user_agent]
      .filter(Boolean)
      .join(' ');
    ...
    for (const { pattern, weight, name } of SQL_PATTERNS) {
    ...
    if (maxScore > 0) {
      threats.push({
        ...
        risk_score:      maxScore,
        description:     `SQL injection attempt: ${matchedNames.join(', ')}`,
        matched_pattern: matchedNames.join(' | '),
